...
Все подтверждающие измерения проекта на 2026-08-26 получены на синтетических датасетах (seed-controlled bit generators), не на реальных доменах. Выводы верны в той мере, в которой синтетика отражает домен — этого нам не известно. Это честно фиксируется в каждом отчёте.
