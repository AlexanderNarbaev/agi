...
## 1.12 Попытка 21 (2026-08-24): margin-gating + трасса каскада

- Margin-gating (усиление только при |vote|<T) внедрён поверх всего стека; toy-гейты зелёные.
- Трасса OR c=8 N=10 e=1200: pos-пул ВЕСЬ вымирает в пустоту через каскад TypeII-ремонтов на негативе (batch includeNow ложных литералов убивает соседние позитивы); neg-veto корректен (¬x0∧¬x1@00). Итог acc=0.25 стабильно.
- Диагноз: каноническое обучение опирается на межклаузную специализацию при БОЛЬШИХ T (сотни клауз) и вероятностной асимметрии, которой нет при малых T. Следующий шаг — dedicated-сессия: сравнение динамики с работающим pyTM side-by-side, фиксация недостающих элементов (взвешенные клаузы, drop_p, T-расписание).

## 1.13 Сетевой аудит закрыт (2026-08-24)

- cair/tmu и tutorial-репозитории: пути недоступны без API-ключа (rate limit); канонические правила УЖЕ вербатим в §4 из ConvolutionalTsetlinMachine.c — этого достаточно для аудита A1–A7 (завершён, семантическая эквивалентность подтверждена).
- Открытый frontier перенесён в карточку EXP-002 как доменно-исследовательская задача следующей сессии: side-by-side динамика против работающего pyTM (требует локальной сборки C-библиотеки).

## 1.16 Попытка 22 (2026-08-24): UPDATE-bootstrap работает

- Разделение режимов UPDATE/PREDICT (D5-refinement): пустые клаузы светятся в обучении → бутстрап заработал: k=16 bacc 0.49–0.60 по сидам (был плоский 0.500).
- Sweep не состоялся технически: exp.* пропы не проброшены в тренировочный путь бенча. След. сессия: починить wiring → сетка s/N/cl/epochs → закрыть stage-1.
