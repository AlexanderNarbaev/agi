...
| **Cauldron (жизненный цикл)** | Cauldron (self-creation protocol) | Протокол самосоздания элементов: циклы рядов нарастающей сложности (МГУА-стиль) → селекция внешним критерием Φ → рождение BirNet/FNL → lineage; память — одна off-heap арена на цикл, пулинг, без GC-штормов (DESIGN-07 §2) |
| **FNL (слой свежих нейронов)** | Fresh Neuron Layer | Реестр «свежих нейронов» — недавно рождённых элементов до интеграции: карантин, теневой прогон, promote/demote через M4-гейты (DESIGN-07 §3) |
