...
| **Расписание Левина** | Universal (Levin) search schedule | Чередование программ с долей времени 2^−l(p) на программу длины l: оптимально для задач инверсии с точностью до мультипликативной константы (Левин 1973); в проекте — эвристика распределения бюджетов рядов Cauldron с Kt-скорингом кандидатов (ALGORITHM-ATLAS-WAVE4 §27, H-019) |
| **Трайрупья** | Trairūpya (три признака валидного основания) | Критерий Дхармакирти (VII в.): основание валидно, если (1) присутствует в субъекте, (2) присутствует только в подобных случаях, (3) полностью («эва») отсутствует в неподобных, и это достоверно установлено («нишчитам»); в проекте — гейт приёма клауз продюсерами (ALGORITHM-ATLAS-WAVE4 §30, H-020) |
| **Звезда** | Star (ограниченная звезда AQ) | Множество максимально общих обобщений зерна (позитивного примера), не покрывающих ни одного негативного (Михальски, AQ, 1969); детерминированное расписание зёрен заменяет случайный выбор (ALGORITHM-ATLAS-WAVE4 §31, H-020) |
| **Сьядвада** | Syādvāda (условная предикация) | Джайнская логика: каждый вердикт префиксован «сьят» («в некотором отношении»); саптабханги — семь предикатов включая авактавья («невыразимо»); в проекте — легитимация UNDECIDED с обязательной меткой standpoint (ALGORITHM-ATLAS-WAVE4 §30) |
| **wp-исчисление** | Weakest precondition calculus | Исчисление Дейкстры (EWD 472, 1975): слабейшее предусловие wp(S, R), условия здравости, вывод программы из постусловия; в проекте — дисциплина correctness-by-construction компилятора форм: сторожа выводятся из инвариантов (ALGORITHM-ATLAS-WAVE4 §28) |
| **SOS** | Structural Operational Semantics | Структурная операционная семантика Плоткина (1981): семантика как система переходов, заданная правилами по структуре синтаксиса, со структурной индукцией; формат семантики BRC и исполнения форм (ALGORITHM-ATLAS-WAVE4 §29) |
| **Институция** | Institution (Бёрстолл–Гоген) | Формализация «логической системы»: сигнатуры + предложения + модели + удовлетворение, инвариантное при морфизмах сигнатур; в проекте — хартия сигнатурной/субстратной независимости BIR (NFR-12, ALGORITHM-ATLAS-WAVE4 §29) |
