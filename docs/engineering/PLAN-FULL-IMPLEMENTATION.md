...
- [W7] partial 2026-08-25 (продолжение): аудит выявил уже реализованные `federation.ArtifactSigner` (Ed25519 sign/verify) и `noosphere` mesh-классы; добавлен недостающий anti-replay `ElspChannel` (seq-монотонность + подпись seq‖payload) с тестами — зелёный. Остаток W7: CRD-манифесты operator.
