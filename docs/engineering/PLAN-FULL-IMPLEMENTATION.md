...
- [W6] ЗАВЕРШЁН в доступной ёмкости 2026-08-25: EXP-010 полный мини-протокол (9 прогонов: 3 датасета × 3 seeds) → medianSpeedup 242.43×, minAdvantage +5 п.п., WiSARD 9/9 → **H-010 accepted (synthetic-scope)**; реализован baseline `evolution.MpdtGaProducer` и выполнен EXP-002 → **H-002 refuted-toy** (GA быстрее ×5.5, точнее до +8.75 п.п., компактнее ×7500); INV-1 реализован как dependency-free source-scan тест `bir.Inv1SourceGuardTest` (выполняется штатным test-таском в CI; workflows не тронуты).
