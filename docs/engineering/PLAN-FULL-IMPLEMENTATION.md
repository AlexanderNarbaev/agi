...
- [W6] partial→прогресс 2026-08-25: EXP-010 выполнены ПЕРВЫЕ РЕАЛЬНЫЕ измерения (Exp010ComparisonTest, seed42, single-run JVM): Tsetlin 10.068ms/acc .5875 vs WiSARD 1.854ms/acc .9250 (5.43×) — отчёт docs/research/reports/EXP-010-report.md; гейты H-010 не закрыты (5.43×<10×), статус running. EXP-002: baseline MPDT-GA trainer отсутствует как подключаемый продюсер — BLOCKED-EXT(impl). INV-1 ArchUnit: dep отсутствует, CI FROZEN — BLOCKED-EXT.
