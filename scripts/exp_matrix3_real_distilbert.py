...
# M-A.T.R.I.X.3: real (non-tiny) DistilBERT distillation.
# - Loads models/external/distilbert-base-sst2/model.onnx (the real
#   distilbert-base-uncased-finetuned-sst-2-english, NOT the tiny variant)
# - Runs inference on a synthetic SST-2 corpus (10 sentences)
# - Captures logits (binary sentiment)
# - Compares latency: torch-CPU vs torch-GPU (cu130) vs MATRIX-BIR eval
# - Measures fidelity: does the model classify the corpus correctly?
#
# Run: python3 scripts/exp_matrix3_real_distilbert.py
import os
import sys
import time
...
# the tiny variant (M-A.T.R.I.X.3).
...
def main() -> int:
    ...
    print(f"[M-A.T.R.I.X.3] torch CUDA available: {has_cuda}")
    ...
    print("[M-A.T.R.I.X.3] torch-CPU inference")
    ...
    print(f"[M-A.T.R.I.X.3] torch-CPU batch=10: total {cpu_batch_us:.1f} μs, per-call {cpu_per_call_us:.1f} μs")

    # ---- B. GPU inference (torch cu130) ----
    if has_cuda:
        print("[M-A.T.R.I.X.3] torch-GPU inference (cu130)")
        ...
        print(f"[M-A.T.R.I.X.3] torch-GPU batch=10: total {gpu_batch_us:.1f} μs, per-call {gpu_per_call_us:.1f} μs")
    else:
        gpu_batch_us = -1
        gpu_per_call_us = -1

    # ---- C. Binarize & build a small BIR approximation ----
    print("[M-A.T.R.I.X.3] binarize logits → tiny BIR distillation")
    ...
    print(f"[M-A.T.R.I.X.3] torch-CPU accuracy on synthetic corpus: {fidelity_cpu:.3f}")

    if has_cuda:
        pred_gpu = (logits_gpu[:, 0] < logits_gpu[:, 1]).astype(int).tolist()
        fidelity_gpu = sum(1 for a, b in zip(true, pred_gpu) if a == b) / len(CORPUS)
        print(f"[M-A.T.R.I.X.3] torch-GPU accuracy on synthetic corpus: {fidelity_gpu:.3f}")
    else:
        fidelity_gpu = -1

    # ---- D. BIR micro-eval timing ----
    print("[M-A.T.R.I.X.3] BIR micro-eval (Java backend via subprocess)")
    bir_per_call_ns = measure_bir_per_call()
    print(f"[M-A.T.R.I.X.3] BIR per-call (Java BooleanRuntime.evaluate): {bir_per_call_ns:.1f} ns")

    # ---- Summary ----
    print()
    print("=" * 64)
    print(f"[M-A.T.R.I.X.3] SUMMARY (10-sentence synthetic SST-2 corpus)")
